    ...
    <script>
        ...
        function guardar(e) {
            if (!control) {
                ...
                control = true;
                Update(user.value, pass.value, id);
            } else {
                document.querySelector(".resultado").textContent="Primero Guardar Cambios";
            }
        }
        ...
        function Update(user, pass, id) {
            Action = "Update";
            var ruta = "Action=" + Action + "&user=" + user + "&pass=" + pass + "&id=" + id;
            $.ajax({
                url: './Vista/VistaGestionUsuario.php',
                type: 'POST',
                data: ruta,
        ...
        function Delete(id, tr) {
            ...
            $.ajax({
                ...
                if (parseInt(respu) === 1) {
                    ...
                    $('.resultado').html("Se Elimino correctamente");
                } else {
                    $('.resultado').html("No Se Elimino correctamente");
                }
            }).fail(function () {
                console.log("error");
            })
